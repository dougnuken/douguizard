...
export default function Scene3D() {
  ...
  return (
    <>
      {/* Layer 1 — Big violet nebulae (Reflect-inspired) */}
      <div
        className="fixed inset-0 z-0 pointer-events-none animate-nebula-drift"
        style={{
          background: `
      ...
      {/* Layer 2 — Decorative stars */}
      <DecorativeStars scrollY={scrollY} />

      {/* Layer 3 — Interstellar shader, very subtle */}
      <div
        className="fixed inset-0 z-[1] pointer-events-none"
        style={{ opacity: 0.35 }}
      >
        <InterstellarShader speed={0.6} opacity={1} />
      </div>

      {/* Layer 4 — Soft vignette only at edges */}
      <div
        className="fixed inset-0 z-[2] pointer-events-none"
        style={{
          background: `
            radial-gradient(ellipse at center, transparent 65%, rgba(21, 17, 45, 0.45) 100%)
          `,
        }}
      />
    </>
